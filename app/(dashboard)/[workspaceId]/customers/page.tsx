...
import { Plus, Search, Filter, Download, TrendingUp, TrendingDown, Users, CreditCard, UserMinus, Eye, MoreVertical } from 'lucide-react';
...
export default function CustomersPage() {
    ...
    const { data, error, isLoading } = useSWR(
        `/api/customers?page=${page}&limit=10${search ? `&search=${search}` : ''}`,
        fetcher
    );
    ...
    const getSubscriptionBadge = (plan: string, status: string) => {
        if (status !== 'active') {
            return <Badge variant="destructive" className="text-xs">Churned</Badge>;
        }

        const variants: Record<string, { variant: any; label: string }> = {
            'Starter': { variant: 'secondary', label: 'Trialing' },
            'Professional': { variant: 'default', label: 'Active • Pro Plan' },
            'Enterprise': { variant: 'default', label: 'Active • Enterprise' }
        };

        const config = variants[plan] || { variant: 'default', label: `Active • ${plan}` };
                            ...
                            <tbody>
                                {isLoading ? (
                                    ...
                                    data?.items?.map((customer: any) => (
                                        <tr key={customer.id} className="border-b hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => window.location.href = `/${workspaceId}/customers/${customer.id}`}>
                                            <td className="p-4">
