...
async def telnyx_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        log_debug(f"Parsed Webhook Data: {json.dumps(data)}")
        ...
        # Try to find workspace_id early to fetch correct API key
        # For inbound calls, 'to' is our number. For outbound calls, 'from' is our number.
        to_number = payload.get("to")
        from_number = payload.get("from")
        found_workspace_id = None
        
        # Check 'to' number first (inbound)
        if to_number:
            phone_record = db.query(PhoneNumber).filter(PhoneNumber.phone_number == to_number).first()
            if phone_record:
                found_workspace_id = phone_record.workspace_id
        
        # If not found, check 'from' number (outbound)
        if not found_workspace_id and from_number:
            phone_record = db.query(PhoneNumber).filter(PhoneNumber.phone_number == from_number).first()
            if phone_record:
                found_workspace_id = phone_record.workspace_id

        telnyx_key = IntegrationService.get_provider_key(workspace_id=found_workspace_id, provider="telnyx", env_fallback="TELNYX_API_KEY")
        client = Telnyx(api_key=telnyx_key)
        
        if event_type == "call.initiated":
             # For inbound calls, we need to answer to get to call.answered state
             direction = payload.get("direction")
             if direction in ["inbound", "incoming"]:
                  logger.info(f"Answering inbound Telnyx call {call_id} using requests")
                  try:
                      ans_url = f"https://api.telnyx.com/v2/calls/{call_id}/actions/answer"
                      ans_resp = requests.post(ans_url, headers={"Authorization": f"Bearer {telnyx_key}", "Content-Type": "application/json"}, json={})
                      log_debug(f"Answer API response: {ans_resp.status_code} - {ans_resp.text}")
                  except Exception as ae:
                      log_debug(f"Answer FAILED: {ae}")
        
        elif event_type == "call.answered":
            # 1. Resolve room name and workspace
            to_num = payload.get("to", "")
            from_num = payload.get("from", "")
            webhook_direction = payload.get("direction", "")
            
            log_debug(f"Handling call.answered: {call_id} | Dir: {webhook_direction} | To: {to_num} | From: {from_num}")

            # Prevent loops: if the 'to' is already a SIP URI, we've already bridged
            if to_num.startswith("sip:"):
                log_debug(f"Ignoring call.answered for SIP leg {call_id} (already bridged)")
                return {"status": "ok"}
            ...
            if comm:
                call_direction = comm.direction
                workspace_id = comm.workspace_id
                safe_comm_id = str(comm.id).replace("_", "-")
                room_name = f"{call_direction}-{safe_comm_id}"
                log_debug(f"Found {call_direction} comm record {comm.id} for call {call_id}")
            else:
                # Fallback: Detect direction from numbers
                p_rec = db.query(PhoneNumber).filter(PhoneNumber.phone_number == to_num).first()
                if p_rec:
                    call_direction = "inbound"
                    workspace_id = p_rec.workspace_id
                else:
                    p_rec = db.query(PhoneNumber).filter(PhoneNumber.phone_number == from_num).first()
                    call_direction = "outbound" if p_rec else "inbound"
                ...
                if call_direction == "inbound":
                    ...
                    comm = Communication(
                        ...
                        user_identifier=from_num,
                        started_at=datetime.utcnow()
                    )
                    db.add(comm)
                    db.commit()
                    safe_comm_id = str(comm_id).replace("_", "-")
                    room_name = f"inbound-{safe_comm_id}"
                else:
                    safe_call_id = call_id.replace(":", "-").replace("_", "-")
                    room_name = f"outbound-{safe_call_id}"
                
                log_debug(f"No comm record found for {call_id}, resolved as {call_direction}, room: {room_name}")

            if comm:
                 comm.status = "answered"
                 comm.telnyx_call_id = call_id
                 log_debug("Committing comm record update to DB...")
                 db.commit()
                 log_debug("Comm record committed successfully.")

            # 3. Perform SIP Transfer
            asterisk_host = os.getenv("ASTERISK_HOST", "147.182.149.234")
            transfer_uri = f"sip:{room_name}@{asterisk_host}"
            log_debug(f"Transfer URI resolved to: {transfer_uri}")
            
            # For inbound calls, we must pre-create the LiveKit room and dispatch the agent
            if call_direction == "inbound":
                try:
                    log_debug("Importing livekit api...")
                    from livekit import api
                    lk_api_key = os.getenv("LIVEKIT_API_KEY")
                    lk_api_secret = os.getenv("LIVEKIT_API_SECRET")
                    lk_url = os.getenv("LIVEKIT_URL")
                    
                    log_debug(f"Checking credentials: URL={bool(lk_url)}, Key={bool(lk_api_key)}")
                    
                    if lk_api_key and lk_api_secret and lk_url:
                        log_debug(f"Pre-creating inbound room {room_name} for agent dispatch")
                        lkapi = api.LiveKitAPI(lk_url, lk_api_key, lk_api_secret)
                        
                        room_metadata = {
                            "communication_id": getattr(comm, 'id', getattr(comm, 'comm_id', None)),
                            "workspace_id": workspace_id,
                        ...
                        await lkapi.aclose()
                        log_debug(f"Pre-created inbound room {room_name} successfully")
                except Exception as e:
                    log_debug(f"WARNING: Failed to pre-create inbound room: {e}")
            
            # Fetch the API key again with the resolved workspace_id
            log_debug(f"Fetching Telnyx API key for workspace {workspace_id}...")
            telnyx_key = IntegrationService.get_provider_key(workspace_id=workspace_id, provider="telnyx", env_fallback="TELNYX_API_KEY")

            if not telnyx_key:
                log_debug(f"CRITICAL: No Telnyx API key found for workspace {workspace_id} or environment!")
            else:
                masked_key = f"{telnyx_key[:5]}...{telnyx_key[-5:]}" if len(telnyx_key) > 10 else "SHORT_KEY"
                log_debug(f"Resolved Telnyx key: {masked_key} (len: {len(telnyx_key)})")

            log_debug(f"Transferring Telnyx call {call_id} to {transfer_uri} (bridging room: {room_name})")
            logger.info(f"Transferring Telnyx call {call_id} to {transfer_uri} (bridging room: {room_name})")
            ...
            try:
                ...
                # Improved SIP headers with correct casing for LiveKit
                request_payload = {
                    "to": transfer_uri,
                    "from": from_number if call_direction == "outbound" else to_number,
                ...
                resp = requests.post(url_endpoint, json=request_payload, headers=headers)
                log_debug(f"Transfer hit API: {resp.status_code} - {resp.text}")
                logger.info(f"Transfer hit API: {resp.status_code} - {resp.text}")
                
            except Exception as te:
                log_debug(f"Transfer FAILED for {call_id}: {te}")
                logger.error(f"Transfer FAILED for {call_id}: {te}")
        ...
        log_debug(f"CRITICAL ERROR in telnyx_webhook: {error_msg}")
...
async def texml_inbound(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        safe_call_id = call_id.replace(":", "-").replace("_", "-")
        room_name = f"inbound-{safe_call_id}"
