    ...
    def get_allowed_tool_names(enabled_skills) -> list:
        ...
        if enabled_skills:
            for skill in enabled_skills:
                raw_mapping = BrainService.SKILL_TO_DIRECT_TOOLS.get(skill.slug, "")
    ...
    def build_prompt(settings, personality_prompt, enabled_skills, workspace_info, current_datetime_str, client_location, agent_type="business", call_context=None):
        ...
        if enabled_skills:
            for skill in enabled_skills:
                ...
                if skill.slug in BrainService.SKILL_TO_DIRECT_TOOLS:
                    direct_tool_list.append(BrainService.SKILL_TO_DIRECT_TOOLS[skill.slug])
        ...
        allowed_list_str = "\n".join([f"- {w}: {BrainService.WORKER_DESCRIPTIONS.get(w, w)}" for w in allowed_workers]) or "- None"
        ...
        # 6. Final Assemble
        full_prompt = f"{BrainService.ACKNOWLEDGEMENT_RULES}\n\n{gatekeeper}\n\n"
